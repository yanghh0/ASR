...
class DataSpeech():
    ...
    def load_data_list(self):
        if self.type == 'train':
            filename_wavlist_thchs30 = os.path.join('datalist', 'train.wav.lst')
            filename_wordlist_thchs30 = os.path.join('datalist', 'train.word.txt')
        elif self.type == 'dev':
            filename_wavlist_thchs30 = os.path.join('datalist', 'cv.wav.lst')
            filename_wordlist_thchs30 = os.path.join('datalist', 'cv.word.txt')
        elif self.type == 'test':
            filename_wavlist_thchs30 = os.path.join('datalist', 'test.wav.lst')
            filename_wordlist_thchs30 = os.path.join('datalist', 'test.word.txt')
        else:
            pass
        self.dic_wavlist , self.list_wavnum = get_wav_list(os.path.join(self.datapath, filename_wavlist_thchs30))
        self.dic_textlist , self.list_textnum = get_wav_text(os.path.join(self.datapath, filename_wordlist_thchs30))
    ...
    def get_text_list(self):
        list_dict = []
        with open('dict_3781', 'r', encoding='UTF-8') as fr:
            lines = fr.readlines()
            for line in lines:
